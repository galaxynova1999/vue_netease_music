       ...
       <div v-else>
         ...
         >
           ...
           <v-card>
             ...
             <v-card-actions>
               <v-spacer/>
               <v-btn text color="primary" @click="loginFormOpen = false">取消</v-btn>
...
export default {
  ...
  methods:{
    ...
    handleLogin() {
      ...
      LoginByPhone(this.phone,this.password).then((response) => {
        if(response.cookie) {
          this.updateProfile(response);
          const { id } = response.account;
          Promise.all([getUserPlayList(id),getUserRecentWeekPlayRecord(id)]).then(([playListData,playRecordData]) => {
