<template>
  <v-container fluid>
    <v-card elevation="0" style="display: flex;align-items: center;justify-content: flex-start">
        ...
        >
          ...
          <template v-slot:item.artist="{ item }">
             <span v-for="(artist,index) of item.ar">
               <span class="list-item" @click="changeRouteToSinger(artist.id)">{{ artist.name }}</span>
          ...
          <template v-slot:item.album="{ item }">
            <span @click="changeRouteToAlbums(item.al.id)" class="list-item">{{item.al.name}}</span>
          </template>
  ...
  methods:{
    ...
    fetchData() {
      ...
      if(type === SONG){
        ...
        this.type = ALBUM;
        Promise.all([getAlbumDetail(id),getAlbumInfo(id)]).then(([detail,info]) => {
